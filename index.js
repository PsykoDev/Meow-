...
module.exports = function autoFishing(mod) {
	const _m = {
		...
		i: {
			n: {
				206400: 'Stone Moroko', //fish R 0-9
	...
	function check() {
		let h = mod.hook('S_INVEN', 19, ({ first, more, size, items }) => {
	...
	function load() {
		...
		hook('C_CAST_FISHING_ROD', 2, ({ dbid, id }) => { _m.cCastFishingRod = { dbid: dbid, id: id }; });

		hook('S_FISHING_BITE', 1, ({ gameId, id }) => {
			if (!mod.game.me.is(gameId)) return;

			_m.t = setTimeout(() => { mod.send('C_START_FISHING_MINIGAME', 2, {counter: 1, unk:15}); }, random(250, 1000));
			return false;
		});
		...
		hook('S_START_FISHING_MINIGAME', 1, ({ gameId, grade }) => {
			...
			_m.t = setTimeout(() => { mod.send('C_END_FISHING_MINIGAME', 2, { pass: true, counter:1, unk:24 }); }, random(3000, 5000) + (grade * 100));
			return false;
		});
		...
		hook('S_SYSTEM_MESSAGE', 1, ({ message }) => { // mod.log(`S_SYSTEM_MESSAGE: message ${message}.`) @4121 = need bait, @4123 = full inv, @3117 = full decomposition list
			...
			if (m === 'SMT_CANNOT_FISHING_NON_BAIT') {
				...
				if (_m.sRequestContract) { mod.send('C_DEL_ITEM', 3, { gameId: mod.game.me.gameId, slot: _m.f_slot - 40, amount: 500 }); }
			}
	...
	function unload() {
		...
		switch (_m.state) {
			case 'cast':
				...
				break;
			case 'minigame':
				mod.send('C_END_FISHING_MINIGAME', 2, { pass: true, counter:1, unk:24 });
				break;
			case 'catch':
			case 'craft':
				break;
